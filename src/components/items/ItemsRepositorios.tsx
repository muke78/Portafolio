...
export const Repositorios: React.FC<PropsRepositorios> = ({ currentLocale }) => {
    ...
    return (
        <>
            {Array.isArray(data) && data.length > 0 ? (
                data.map((repo) => (
                    <div key={repo.id} className="backdrop-blur-[19px] backdrop-saturate-[180%] border border-neutral-700 rounded-lg py-5 card shadow-md p-4 flex flex-col justify-between h-full">
                            ...
                            {repo.topics ? (
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    {repo.topics.map((topic) => (
                                        <span key={topic} className="bg-gray-800 text-white text-sm px-3 py-1 rounded-full">
                                            {topic}
                                        </span>
                                    ))}
                                </div>
